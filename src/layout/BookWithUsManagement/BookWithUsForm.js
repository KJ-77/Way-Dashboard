...
const BookWithUsForm = () => {
    ...
    if (isEditMode) {
      const fetchItem = async () => {
        try {
          ...
          const response = await fetch(`${BASE_URL}/api/book-with-us/${slug}`, {
            method: 'GET',
            headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response
      if (isEditMode) {
        response = await fetch(`${BASE_URL}/api/book-with-us/${slug}`, {
          method: 'PUT',
        ...
        response = await fetch(`${BASE_URL}/api/book-with-us`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` },
          body: formDataToSend,
        })
      ...
      if (data.success) {
        ...
        setTimeout(() => {
          navigate('/book-with-us-management')
        }, 1500)
      } else {
  ...
  const handleBack = () => navigate('/book-with-us-management')
    ...
    <CCard>
      <CCardHeader>
        <CRow>
          <CCol>
            <h5>
              {isEditMode ? 'Edit Book With Us' : 'Create New Book With Us'}
            </h5>
          </CCol>
          <CCol xs="auto">
