...
const ProductionList = () => {
  ...
  // Load items on component mount and when pagination/search changes
  useEffect(() => {
    loadItems()
  ...
  const loadItems = async (page = 1, search = '') => {
    try {
      setLoading(true)
      setError(null)
      // Get all productions from API
      const response = await fetch(`${BASE_URL}/api/production`)
      const data = await response.json()

      if (!response.ok) throw new Error(data.message || 'Failed to load items')

      // Handle client-side filtering since pagination might not be implemented on backend
  ...
  const handleEdit = (item) => {
    navigate(`/production-management/${item.slug}/edit`)
  }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/api/production/${selectedItem.slug}`,
        {
          method: 'DELETE',
  ...
  return (
    <CRow>
      <CCol xs={12}>
        ...
        {loading ? (
          ...
          <CCard>
            <CCardBody>
              <div className="text-center mt-3">
                <h5>No Production Items Found</h5>
                ...
                <CButton
                  color="primary"
                  onClick={() => navigate('/production-management/new')}
                >
          ...
          <CCard>
            <CCardHeader className="d-flex justify-content-between align-items-center">
              <h5 className="mb-0">Production Management</h5>
              <CButton
                color="primary"
                onClick={() => navigate('/production-management/new')}
              >
        ...
        >
          <CModalHeader>
            <CModalTitle>Production Details</CModalTitle>
          </CModalHeader>
          <CModalBody>
            {selectedItem && (
              <div>
